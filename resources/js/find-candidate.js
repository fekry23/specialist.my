...
checkboxBtns.forEach(checkboxBtn => {
    checkboxBtn.addEventListener('change', function () {
        const selectedRate = Array.from(document.querySelectorAll('input[name="rate"]:checked')).map(el => el.value).join(',');
        const selectedLevel = Array.from(document.querySelectorAll('input[name="level"]:checked')).map(el => el.value).join(',');
